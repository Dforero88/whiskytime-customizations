...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'aeicmsbottominfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'aeicmsbottominfo_lang` il ON il.`id_aeicmsbottominfo` = i.`id_aeicmsbottominfo`'
        );
    ...
    public function insertData()
    {
        ...
        $AEICmsBottoms = array_intersect_key($this->loadedData, $shopsIds);

        $aeicmsbottominfo = new CustomText();
        $aeicmsbottominfo->text = reset($AEICmsBottoms);
        $return &= $aeicmsbottominfo->add();

        if (count($AEICmsBottoms) > 1) {
            foreach ($AEICmsBottoms as $key => $text) {
                Shop::setContext(Shop::CONTEXT_SHOP, (int) $key);
                $aeicmsbottominfo->text = $text;
                $return &= $aeicmsbottominfo->save();
            }
        }
