...
include_once(_PS_MODULE_DIR_.'aei_leftbanner/AEILeftBanner.php');

class Aei_LeftBanner extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('AX Left Banner', array(), 'Modules.AEILeftBanner');
        $this->description = $this->getTranslator()->trans('Display banner on left column in your store.', array(), 'Modules.AEILeftBanner');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:aei_leftbanner/views/templates/hook/aeileftbanner.tpl';
    }
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 1; ++$i) {
            $slide = new AEILeftBanner();
            $slide->position = $i;
            $slide->active = 1;
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'AEILeftBanner '.$i;                
                $slide->url[$language['id_lang']] = '#';
                $slide->image[$language['id_lang']] = 'left-banner-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeileftbanner` (
                `id_aeileftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_aeileftbanner_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeileftbanner_slides` (
              `id_aeileftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_aeileftbanner_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeileftbanner_slides_lang` (
              `id_aeileftbanner_slides` int(10) unsigned NOT NULL,
              `id_lang` int(10) unsigned NOT NULL,
              `title` varchar(255) NOT NULL,
              `url` varchar(255) NOT NULL,
              `image` varchar(255) NOT NULL,
              PRIMARY KEY (`id_aeileftbanner_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new AEILeftBanner($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'aeileftbanner`, `'._DB_PREFIX_.'aeileftbanner_slides`, `'._DB_PREFIX_.'aeileftbanner_slides_lang`;
        ');
    }
    ...
    protected function _postValidation()
    {
        ...
        /* Validation for Slide configuration */
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_slide')) {
            $slide = new AEILeftBanner((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new AEILeftBanner((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'aeileftbanner (id_aeileftbanner_slides, id_shop)
            SELECT id_aeileftbanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'aeileftbanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'aeileftbanner_slides` hss, `'._DB_PREFIX_.'aeileftbanner` hs
            WHERE hss.`id_aeileftbanner_slides` = hs.`id_aeileftbanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_aeileftbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`image`
            FROM '._DB_PREFIX_.'aeileftbanner hs
            LEFT JOIN '._DB_PREFIX_.'aeileftbanner_slides hss ON (hs.id_aeileftbanner_slides = hss.id_aeileftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'aeileftbanner_slides_lang hssl ON (hss.id_aeileftbanner_slides = hssl.id_aeileftbanner_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'aei_leftbanner/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'aeileftbanner_slides hss ON (hs.id_aeileftbanner_slides = hss.id_aeileftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'aeileftbanner_slides_lang hssl ON (hss.id_aeileftbanner_slides = hssl.id_aeileftbanner_slides)
            WHERE hs.`id_aeileftbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_aeileftbanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'aeileftbanner` hs
                WHERE hs.`id_aeileftbanner_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = AEILeftBanner::getAssociatedIdsShop((int)$slide['id_slide']);
        ...
        return $this->display(__FILE__, 'slidelist.tpl');
    }
