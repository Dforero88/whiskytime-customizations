...
{
    $module = $module;
    if (columnExist('image_status')
    && columnExist('event_row')
    && columnExist('event_seat')
    && columnExist('pdf_status')
    && columnExist('event_row_status')
    && columnExist('event_seat_status')) {
        return true;
    } else {
        Db::getInstance()->execute('ALTER TABLE `' . _DB_PREFIX_ . 'fme_events`
            ADD `image_status` smallint(6) default 0,
            ADD `pdf_status` smallint(6) default 0');
    }
