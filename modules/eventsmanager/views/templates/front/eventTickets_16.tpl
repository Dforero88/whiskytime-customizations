...
<div class="fmm_block front_panel" style="overflow-x:auto;background: white;">
...
<table class="table table-hover" style="font-size: smaller;">
  <thead>
    <tr>
      ...
      <th scope="col">{l s='Customer Name' mod='eventsmanager'}</th>
      <th scope="col">{l s='Customer Phone' mod='eventsmanager'}</th>
  ...
  <tbody>
    {foreach from=$alldata item=data}
     <tr>
          <td>{Events::getProductNameLang($data.id_product|escape:'htmlall':'UTF-8')}</td>
          ...
          <td>{$data.customer_name|escape:'htmlall':'UTF-8'}</td>
          <td>{$data.customer_phone|escape:'htmlall':'UTF-8'}</td>
          <td>
            {if $data.order_status == 0 || $data.order_status == 1 || $data.order_status == 10}
          ...
          <td>
            ...
            {else}
                <a style="margin-top: -5px;" class="btn btn-default" href="{$ajax_url|escape:'htmlall':'UTF-8'}&id_order={$data.id_order|escape:'htmlall':'UTF-8'}"><i class="material-icons">cloud_download</i></a>
            {/if}
          </td>
          <td>
