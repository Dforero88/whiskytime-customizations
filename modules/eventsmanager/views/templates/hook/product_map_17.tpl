...
{foreach from=$match_products item=id_product}
  ...
  <!--  <h5 class="p_name">{Product::getProductName({$id_product.id_product|escape:'htmlall':'UTF-8'})|escape:'htmlall':'UTF-8'}</h5> -->
   ...
   <div class="col-lg-12">
    ...
    {if $isseatmap}
      {if $ps_version < 1.7}
      <button style="margin-top: 5px;" type="button" name="selectMap" id="select_seat_{$id_product.event_id|escape:'htmlall':'UTF-8'}" onclick="openSelectMap({$id_product.event_id|escape:'htmlall':'UTF-8'},{$name[0]['cart_quantity']|escape:'htmlall':'UTF-8'}, {$id_product.id_product|escape:'htmlall':'UTF-8'}, {$id_product.event_product_id|escape:'htmlall':'UTF-8'});" class="btn btn-primary btn-xs" comp="">{l s='Select Seat' mod='eventsmanager'}</button>
      {else}
        <button style="margin-top: 5px;" type="button" name="selectMap" id="select_seat_{$id_product.event_id|escape:'htmlall':'UTF-8'}" data-toggle="modal" data-target="#openMap" onclick="openSelectMapProduct({$id_product.event_id|escape:'htmlall':'UTF-8'},{$name['quantity']|escape:'htmlall':'UTF-8'}, {$id_product.id_product|escape:'htmlall':'UTF-8'}, {$id_product.event_product_id|escape:'htmlall':'UTF-8'});" class="btn btn-primary btn-xs" comp="">{l s='Select Seat' mod='eventsmanager'}</button>
      {/if}
...
<div class="modal fade" id="openMap" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
      <div class="modal-content" style="overflow: scroll;padding-left: 20px;background-color: #e6e6e6;">
        <h2 style="text-align: center;margin-top: 10px;">{l s='Select Seat' mod='eventsmanager'}</h2>
        ...
        <div class="clearfix"></div>
         ...
         <span id="cart_message" style="display: none;color: red;">{l s='Kindly try Later: ' mod='eventsmanager'}</span>
