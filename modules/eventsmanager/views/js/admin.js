...
function reserveBookedSelectMap(id_event, id_product, event_product_id) {
    $('#seat_maphide').remove();
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function(data) {
            $('#show_map').html('<input type="hidden" id="ideventproduct_ajax" value="' + event_product_id + '"><input type="hidden" id="ajax_id_product" value="' + id_product + '"><input type="hidden" id="' + id_event + '_select_seat" value="0"><input type="hidden" id="idevent_ajax" value="' + id_event + '"><table border="1" id="row_col_table">' + data[0] + '</table>');
            $('#openMap').modal('show');
            var str_all = data[1];
...
$(document).on('click', ".sello", function() {
    ...
    var id_event = $("#idevent_ajax").val();
    var added_value = $('#' + id_event + '_select_seat').val();
    if (added_value && added_value > 0) {
        // $('#' + id_event + '_select_seat').val(selected_value.count());
        $('#' + id_event + '_select_seat').val(added_value + ',' + selected_value);
    } else {
        $('#' + id_event + '_select_seat').val(selected_value);
    }
