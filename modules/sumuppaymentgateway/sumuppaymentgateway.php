...
class Sumuppaymentgateway extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->moduleToken = method_exists('Tools', 'encrypt')
            ? Tools::encrypt($this->name . _COOKIE_KEY_)
            : Tools::hash($this->name . _COOKIE_KEY_);
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSumuppaymentgatewayReset')) {
            ...
            $tempFile = tmpfile();
            $tempBasePath = pathinfo(stream_get_meta_data($tempFile)['uri'], PATHINFO_DIRNAME) . DIRECTORY_SEPARATOR;
            fclose($tempFile);

            $logoBasePath = realpath(_PS_MODULE_DIR_ . $this->name . '/views/img/payment_images/') . DIRECTORY_SEPARATOR;

            if (isset($_FILES['SUMUP_LOGO']) && !empty($_FILES['SUMUP_LOGO']['tmp_name'])) {
                if ($error = ImageManager::validateUpload($_FILES['SUMUP_LOGO'], 4000000)) {
                    ...
                    if ($logoPath && strpos($logoPath, realpath($logoBasePath)) === 0) {
                        if (Configuration::get('SUMUP_LOGO') != $this->getDefaultLogo() && $this->validatePath($logoPath, $logoBasePath)) {
                            @unlink($logoPath);
                        }
                    } else {
                        $this->errors[] = Tools::displayError($this->l('Cannot parse SumUp files'));
            ...
            if (isset($_FILES['SUMUP_CREDENTIALS']) && !empty($_FILES['SUMUP_CREDENTIALS']['tmp_name'])) {
                ...
                if ($this->validatePath($credentialsPath, $tempBasePath)) {
                    $sumup_credentials = @file_get_contents($credentialsPath);
                    if ($sumup_credentials === false) {
                        $this->errors[] = Tools::displayError($this->l('Unable to read the SumUp credentials file.'));
                    } else {
        ...
        foreach ($this->errors as $error) {
            PrestaShopLogger::addLog($error, 3, null, 'Module');
        }
    ...
    private function validatePath($path, $basePath)
    {
        if ($path && strpos($path, $basePath) === 0) {
            if (file_exists($path)) {
                ...
                PrestaShopLogger::addLog($this->getErrorMessage('setup_err') . $path . ' | ' . $basePath, 3, null, 'Module');
