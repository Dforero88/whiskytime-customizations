...
class aei_categoryslider extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('AX Category Product Slider', array(), 'Modules.CategorySlider');
        $this->description = $this->trans('Add product categorywise in tab.', array(), 'Modules.CategorySlider');
		
        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
		$this->templateFile = 'module:aei_categoryslider/views/templates/front/aei_categoryslider.tpl';
    }
	
	public function install()
    {	
		$arrayDefault = array('3','4','5','6');
        Configuration::updateValue("AEI_CATE_SLIDER_LIST", serialize($arrayDefault));
		Configuration::updateValue("AEI_CATE_SLIDER", 0);
		Configuration::updateValue("AEI_CATE_NBR", 5);
	...
	public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeicategoryslider` (
			`id_aeicategoryslider` int(11) NOT NULL AUTO_INCREMENT,
			`image` varchar(128) NOT NULL,
			`id_shop` int(10) NOT NULL,
			`name_category` varchar(128) NOT NULL,
			`id_category` int(10)  NOT NULL,		
			PRIMARY KEY (`id_aeicategoryslider`)
    ...
    {
        Configuration::deleteByName('AEI_CATE_SLIDER_LIST');
		Configuration::deleteByName('AEI_CATE_SLIDER');
		Configuration::deleteByName('AEI_CATE_NBR');
		
		$ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'aeicategoryslider`');
        }
    ...
    {
        $output = '';
        if (((bool)Tools::isSubmit('submitAeicategoryslider')) == true) {
            $ids = Tools::getValue('AEI_CATE_SLIDER_LIST');
            $nbr = Tools::getValue('AEI_CATE_NBR');
            ...
            {
				Configuration::updateValue('AEI_CATE_SLIDER', (int)(Tools::getValue('AEI_CATE_SLIDER')));
                Configuration::updateValue('AEI_CATE_NBR', (int)($nbr));
	            Configuration::updateValue('AEI_CATE_SLIDER_LIST', serialize($ids));
        ...
        if (Tools::isSubmit('deleteaei_categoryslider') && Tools::getValue('id_category')) {
    ...
    protected function renderForm()
    {
        ...
        $var = $values['AEI_CATE_SLIDER_LIST'];
        if (!is_array($var)) {
            $var = array(1);
        }
        ...
        $helper->submit_action = 'submitAeicategoryslider';
    ...
    {
        $result =  array(
            'AEI_CATE_SLIDER_LIST' =>  Tools::getValue('AEI_CATE_SLIDER_LIST', Configuration::get('AEI_CATE_SLIDER_LIST')),
            'AEI_CATE_SLIDER' =>  Tools::getValue('AEI_CATE_SLIDER', Configuration::get('AEI_CATE_SLIDER')),
			'AEI_CATE_NBR' =>  Tools::getValue('AEI_CATE_NBR', Configuration::get('AEI_CATE_NBR')),
			'category_id' => Tools::getValue('category_id', Configuration::get('category_id')),
        );

        if(!is_array($result['AEI_CATE_SLIDER_LIST']) && !empty($result['AEI_CATE_SLIDER_LIST'])) {
            $result['AEI_CATE_SLIDER_LIST'] = unserialize($result['AEI_CATE_SLIDER_LIST']);
        }
    ...
    {
        $result = Db::getInstance()->execute('INSERT  INTO `'._DB_PREFIX_.'aeicategoryslider`(`id_category`,`name_category`,`image`,`id_shop`) VALUES ('.(int)$category_id.', \''.pSQL($name_cate).'\', \''.pSQL($filename).'\','.(int)$id_shop.')');
        return $result;
    }

    public function updatecategoryicon($category_id,$name_cate,$filename)
    {
        $result = Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'aeicategoryslider` SET `name_category` = \''.pSQL($name_cate).'\',`image` =\''.pSQL($filename).'\' WHERE `id_category` = '.(int)$category_id);
        return $result;
    }

    public static function deleteCategoryId($category_id)
    {
        $sql = 'DELETE FROM`'._DB_PREFIX_.'aeicategoryslider` WHERE `id_category` = '.(int)$category_id;
        Db::getInstance()->execute($sql);
    }
    
    public function getimage($category_id,$id_shop)
    {
        $sql = 'SELECT id_aeicategoryslider ,image,id_category,name_category FROM '._DB_PREFIX_.'aeicategoryslider WHERE id_category = '.(int)$category_id.' and id_shop ='.(int)$id_shop.'' ;
        return Db::getInstance()->executeS($sql);
    }

    public function getcategoryicon()
    {
        $sql = 'SELECT id_aeicategoryslider ,image,id_category,name_category, id_shop FROM '._DB_PREFIX_.'aeicategoryslider';
        return Db::getInstance()->executeS($sql);
    }
	
	public function hookdisplayHeader($params)
    {
        $this->context->controller->registerJavascript('modules-aeicategoryslider', 'modules/'.$this->name.'/views/js/aei_categoryslider.js', ['position' => 'top', 'priority' => 150]);
		$this->context->controller->registerStylesheet('modules-aeicategoryslider', 'modules/'.$this->name.'/views/css/aei_categoryslider.css', ['media' => 'all', 'priority' => 150]);
    }
	
    public function renderWidget($hookName = null, array $configuration = [])
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('aei_categoryslider'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('aei_categoryslider'));
    }
...
$result = array_slice($result, 0, ($catnb ? $catnb : 8));
        ...
        return array(
             'aeicategorysliderinfos' => $arrayCategory,
			 'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'aei_categoryslider/views/img'),
			 'slider' => (int)Configuration::get('AEI_CATE_SLIDER'),
        );
    }
