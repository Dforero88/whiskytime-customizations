...
public function getWidgetVariables($hookName = null, array $configuration = array())
{
    ...
    $defaultImageName = $isoCode . '-default';
    $defaultImagePath = _PS_MANU_IMG_DIR_ . $defaultImageName . '.jpg';
    $defaultImageExists = file_exists($defaultImagePath);

    foreach ($brands as &$brand) {
        $brand['image'] = $defaultImageName;
        $brand['link'] = $this->context->link->getManufacturerLink($brand);

        $customImagePath = _PS_MANU_IMG_DIR_ . $brand['id_manufacturer'] . '-' . ImageType::getFormattedName('medium') . '.jpg';
        $customImageExists = file_exists($customImagePath);

        // Obtenir le nombre de produits de la marque
        $product_count = Manufacturer::getProducts(
            ...
            'name',     // champ valide
            'asc',      // ordre valide
            true        // on veut juste le count
        );

        $brand['product_count'] = (int)$product_count;

        // Ne conserver que les marques ayant une image personnalisée ou image par défaut
        if ($customImageExists) {
            $brand['image'] = $brand['id_manufacturer'];
            $filteredBrands[] = $brand;
        } elseif (in_array($defaultImageName, ['fr-default', 'en-default']) && $defaultImageExists) {
            $filteredBrands[] = $brand;
        }
    }

    // Tri décroissant par nombre de produits
