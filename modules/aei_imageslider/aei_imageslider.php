...
include_once(_PS_MODULE_DIR_.'aei_imageslider/Aei_HomeSlide.php');

class Aei_ImageSlider extends Module implements WidgetInterface
{
    protected $_html = '';
    protected $default_speed = 3000;
    protected $default_pause_on_hover = 1;
    protected $templateFile;

    public function __construct()
    {
        $this->name = 'aei_imageslider';
        ...
        $this->displayName = $this->getTranslator()->trans('AX Image slideshow', array(), 'Modules.ImageSlider');
        $this->description = $this->getTranslator()->trans('Display Image Slider on Homepage.', array(), 'Modules.ImageSlider');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:aei_imageslider/views/templates/hook/slider.tpl';
    }
    ...
    public function install()
    {
        ...
        if (parent::install() &&
			...
			$this->registerHook('displayFullWidth')
        ) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('AEIHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id, $shop_id);
            }

            /* Sets up Shop Group configuration */
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    $res &= Configuration::updateValue('AEIHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id);
                    $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id);
                }
            }

            /* Sets up Global configuration */
            $res &= Configuration::updateValue('AEIHOMESLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('AEIHOMESLIDER_SPEED');
            $res &= Configuration::deleteByName('AEIHOMESLIDER_PAUSE_ON_HOVER');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeihomeslider` (
                `id_aeihomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_aeihomeslider_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeihomeslider_slides` (
              `id_aeihomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_aeihomeslider_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'aeihomeslider_slides_lang` (
              `id_aeihomeslider_slides` int(10) unsigned NOT NULL,
              ...
              PRIMARY KEY (`id_aeihomeslider_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new Aei_HomeSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'aeihomeslider`, `'._DB_PREFIX_.'aeihomeslider_slides`, `'._DB_PREFIX_.'aeihomeslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = Aei_HomeSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('AEIHOMESLIDER_SPEED'))) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('AEIHOMESLIDER_SPEED', (int)Tools::getValue('AEIHOMESLIDER_SPEED'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('AEIHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id, $shop_id);
            }

            /* Update global shop context if needed*/
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('AEIHOMESLIDER_SPEED', (int)Tools::getValue('AEIHOMESLIDER_SPEED'));
                    $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('AEIHOMESLIDER_PAUSE_ON_HOVER'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue('AEIHOMESLIDER_SPEED', (int)Tools::getValue('AEIHOMESLIDER_SPEED'), false, $shop_group_id);
                            $res &= Configuration::updateValue('AEIHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('AEIHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id);
                        }
                    }
                    break;
            ...
            $slide = new Aei_HomeSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new Aei_HomeSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
        $this->context->controller->registerStylesheet('modules-aeihomeslider', 'modules/'.$this->name.'/views/css/aeislider.css', ['media' => 'all', 'priority' => 140]);
        $this->context->controller->registerJavascript('modules-responsiveslides', 'modules/'.$this->name.'/views/js/responsiveslides.min.js', ['position' => 'bottom', 'priority' => 140]);
		$this->context->controller->registerJavascript('modules-aeihomeslider', 'modules/'.$this->name.'/views/js/aeislider.js', ['position' => 'bottom', 'priority' => 140]);
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        return [
            'aeihomeslider' => [
                'speed' => $config['AEIHOMESLIDER_SPEED'],
                'pause' => $config['AEIHOMESLIDER_PAUSE_ON_HOVER'] ? 'true' : 'false',
                'slides' => $slides,
            ],
        ];
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'aeihomeslider (id_aeihomeslider_slides, id_shop)
            SELECT id_aeihomeslider_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'aeihomeslider
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'aeihomeslider_slides` hss, `'._DB_PREFIX_.'aeihomeslider` hs
            WHERE hss.`id_aeihomeslider_slides` = hs.`id_aeihomeslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_aeihomeslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'aeihomeslider hs
            LEFT JOIN '._DB_PREFIX_.'aeihomeslider_slides hss ON (hs.id_aeihomeslider_slides = hss.id_aeihomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'aeihomeslider_slides_lang hssl ON (hss.id_aeihomeslider_slides = hssl.id_aeihomeslider_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'aei_imageslider/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'aeihomeslider_slides hss ON (hs.id_aeihomeslider_slides = hss.id_aeihomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'aeihomeslider_slides_lang hssl ON (hss.id_aeihomeslider_slides = hssl.id_aeihomeslider_slides)
            WHERE hs.`id_aeihomeslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_aeihomeslider_slides` as id_slide
                FROM `'._DB_PREFIX_.'aeihomeslider` hs
                WHERE hs.`id_aeihomeslider_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = Aei_HomeSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'AEIHOMESLIDER_PAUSE_ON_HOVER',
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'AEIHOMESLIDER_SPEED' => Tools::getValue('AEIHOMESLIDER_SPEED', Configuration::get('AEIHOMESLIDER_SPEED', null, $id_shop_group, $id_shop)),
            'AEIHOMESLIDER_PAUSE_ON_HOVER' => Tools::getValue('AEIHOMESLIDER_PAUSE_ON_HOVER', Configuration::get('AEIHOMESLIDER_PAUSE_ON_HOVER', null, $id_shop_group, $id_shop)),
        );
    }
