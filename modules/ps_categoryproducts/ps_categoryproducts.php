...
class Ps_Categoryproducts extends Module implements WidgetInterface
{
    ...
    private function getCategoryProducts($idProduct, $idCategory)
    {
        ...
        $displayProducts = (int) Configuration::get('CATEGORYPRODUCTS_DISPLAY_PRODUCTS');
        $nProducts = ($displayProducts * 3) + 1; // +1 if current product is found
        ...
        foreach ($products as $rawProduct) {
            $currentProductId = (int) $rawProduct['id_product'];

            if ($currentProductId === (int) $idProduct) {
                continue;
            }

            if (StockAvailable::getQuantityAvailableByProduct($currentProductId) <= 0) {
                continue;
            }

            if (count($productsForTemplate) >= $displayProducts) {
                break;
            }
