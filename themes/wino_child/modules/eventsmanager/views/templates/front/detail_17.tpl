{*
 * Theme override for eventsmanager event detail page.
 * Keeps module logic intact while aligning the presentation with the child theme.
*}
...
{if $default_theme eq 0}
  ...
  <div class="fme-bootstrap wtevent-detail-page">
    ...
    <section class="wtevent-detail">
      <header class="wtevent-detail__hero">
        ...
        <div class="wtevent-detail__panel">
          ...
          {if $enable_tags}
            <div class="wtevent-detail__tags">
              {foreach from=$eventTags item=tag}
                {assign var=params value=['id_tag'=> $tag.id_fme_tags, 'tags'=> $tag.friendly_url]}
                <a class="wtevent-detail__tag" href="{$link->getModuleLink('eventsmanager', 'eventstag', $params)|escape:'htmlall':'UTF-8'}">{$tag.name|escape:'htmlall':'UTF-8'}</a>
              {/foreach}
            </div>
          {/if}
          ...
          {if $twitter_link OR $instagram_link OR $facebook_link}
            <div class="wtevent-detail__socials">
              {if $twitter_link}<a href="{$twitter_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-twitter" aria-label="Twitter"></a>{/if}
              {if $instagram_link}<a href="{$instagram_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-instagram" aria-label="Instagram"></a>{/if}
              {if $facebook_link}<a href="{$facebook_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-facebook" aria-label="Facebook"></a>{/if}
            </div>
          {/if}

          {if $stream_on eq 1}
            {if $eventData.event_streaming_start_time le $eventData.event_streaming_end_time}
              <div class="wtevent-detail__streaming">
                ...
                <p>
                  <strong>{l s='This Streaming is available from' mod='eventsmanager'}:</strong>
                  {$eventData.event_streaming_start_time|escape:'htmlall':'UTF-8'}
                  <strong>{l s='To' mod='eventsmanager'}:</strong>
                  {$eventData.event_streaming_end_time|escape:'htmlall':'UTF-8'}
                </p>
              </div>
            {else}
              <div class="wtevent-detail__streaming wtevent-detail__streaming--ended">
                <strong>{l s='This streaming is no longer available' mod='eventsmanager'}</strong>
              </div>
            {/if}
          {/if}
        </div>
      ...
      {if !empty($eproduct)}
        <section class="wtevent-detail__section">
          <h2 class="wtevent-detail__section-title">{l s='Tickets' mod='eventsmanager'}</h2>
          <div class="wtevent-tickets">
            {foreach from=$eproduct item=product}
              <article class="wtevent-ticket">
                <div class="wtevent-ticket__media">
                  <img src="{$link->getImageLink($cover[$product->id].id_image, $cover[$product->id].id_image, 'small_default')|escape:'htmlall':'UTF-8'}" alt="{$product->name|escape:'htmlall':'UTF-8'}">
                </div>
                <div class="wtevent-ticket__body">
                  <h3 class="wtevent-ticket__name">{$product->name|escape:'htmlall':'UTF-8'}</h3>
                  <div class="wtevent-ticket__price"><strong>{$currency.sign|escape:'html':'UTF-8'} {Product::getPriceStatic($product->id)|string_format:"%.2f"|escape:'html':'UTF-8'}</strong></div>
                </div>
                <div class="wtevent-ticket__actions">
                    ...
                    {else}
                      <form action="{$link->getPageLink('cart', true, NULL, "token={$static_token|escape:'htmlall':'UTF-8'}&amp;add=1&id_product={$product->id|intval}&id_product_attribute=0")|escape:'html':'UTF-8'}" method="post" class="wtevent-ticket__form">
                        ...
                        <button
                          ...
                          type="submit"
                          class="btn btn-primary cart ajax_add_to_cart_button add-to-cart"
                          href="{$link->getPageLink('cart', true, NULL, "token={$static_token|escape:'htmlall':'UTF-8'}&amp;add=1&id_product={$product->id|intval}&id_product_attribute=0")|escape:'html':'UTF-8'}"
                          rel="nofollow"
      ...
      {if $eventGallery|@count > 0}
        {if $events_show_gallery eq 1}
          {if $eventGallery neq ""}
            <section class="wtevent-detail__section">
              <h2 class="wtevent-detail__section-title">{l s='Event Gallery' mod='eventsmanager'}</h2>
              <div class="wtevent-detail__gallery">
      ...
      {if $events_show_youtbue_video eq 1}
        {if $videoId neq ""}
          {if $has_match eq 1}
            <section class="wtevent-detail__section">
              <h2 class="wtevent-detail__section-title">{l s='Event Video' mod='eventsmanager'}</h2>
              <div class="wtevent-detail__video">
                <iframe id="events-video" class="youtube-player" type="text/html" src="https://www.youtube.com/embed/{$videoId[1]|escape:'htmlall':'UTF-8'}" frameborder="0"></iframe>
              </div>
            </section>
          {/if}
        {/if}
      ...
      {if $events_sharing_options eq 1}
        <div class="wtevent-detail__share">
            ...
            <div class="event_info">
              ...
              {if $twitter_link OR $instagram_link OR $facebook_link}
                <div class="event-info-field clearfix">
                  ...
                  <div class="event-info-content">
                    {if $twitter_link}<a href="{$twitter_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-twitter"></a>{/if}
                    {if $instagram_link}<a href="{$instagram_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-instagram"></a>{/if}
                    {if $facebook_link}<a href="{$facebook_link|escape:'htmlall':'UTF-8'}" target="_blank" class="fa fa-facebook"></a>{/if}
                  </div>
                </div>
              {/if}
            </div>
                  ...
                  {if $videoId neq ""}
                    {if $has_match eq 1}
                      <li>
                        ...
                        <div class="video">
                          <iframe id="events-video" class="youtube-player col-lg-12" type="text/html" src="https://www.youtube.com/embed/{$videoId[1]|escape:'htmlall':'UTF-8'}" frameborder="0"></iframe>
                        </div>
                      </li>
                    {/if}
                  {/if}
